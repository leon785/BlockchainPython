...
//#include <libp2p/protocol/FileTransfer.h>
...
class Txc15 : public cSimpleModule
{
  private:
    ...
    string savedInFile = "dataOutput\\dataTxc15.txt";
...
void Txc15::initialize()
{
    ...
    // clear the output file if exits
    struct stat buffer;
    if ( stat(savedInFile.c_str(), &buffer) == 0 ){
        ofstream file_writer(savedInFile, ios_base::out);
        file_writer.close();
    }
...
void Txc15::handleMessage(cMessage *msg)
{
    ...
    if (ttmsg->getDestination() == getIndex()) {
        ...
        ofstream oFile;
        oFile.open(savedInFile, ios::app);
        oFile << hopCountVector.getValuesStored() << "\n";
        oFile.close();

        // stop when tic[0] has received the msg for 10 times
        if (numReceived >= 10) {
            endSimulation();
        }
